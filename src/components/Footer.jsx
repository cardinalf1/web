      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
        ...
        <div className="flex flex-row flex-wrap justify-center gap-6 items-center">
          {data.links?.map((link, idx) => (
            <a key={idx} href={link.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-xs font-code text-gray-400 hover:text-signal transition-colors tracking-widest whitespace-nowrap py-2">
              {link.label}
            </a>
          ))}
          
          {/* Custom Contact Click Menu */}
          <div className="relative py-2 contact-menu-container flex items-center">
            <button 
              onClick={() => setIsContactOpen(!isContactOpen)}
              className={`inline-flex items-center text-xs font-code tracking-widest uppercase focus:outline-none transition-colors duration-300 whitespace-nowrap ${isContactOpen ? "text-signal" : "text-gray-400 hover:text-signal"}`}
            >
              {isContactOpen ? "[ CLOSE_CONTACT ]" : "[ CONTACT ]"}
            </button>
            <div 
              className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-3 flex flex-col bg-void border border-structure/50 p-4 rounded shadow-[0_-10px_35px_rgba(0,0,0,0.95)] w-[90vw] max-w-[360px] z-50 text-left transition-all duration-300 origin-bottom ${
                isContactOpen 
              ...
              <div className="group/item flex flex-col gap-1 hover:bg-white/5 p-2 rounded transition-colors relative border border-transparent hover:border-structure/30">
                <div className="flex justify-between items-center gap-4">
                  ...
                  <button 
                    ...
                    className={`text-[8px] font-code px-2 py-1.5 border transition-all duration-300 rounded uppercase tracking-[0.2em] select-none flex-shrink-0 ${copiedId === "raghav" ? "bg-signal text-void border-signal" : "bg-void border-structure/50 text-gray-500 hover:border-signal hover:text-signal shadow-[0_0_10px_rgba(255,255,255,0.02)]"}`}
                    style={{ minWidth: '56px' }}
                    title="Copy email"
                  >
              ...
              <div className="group/item flex flex-col gap-1 hover:bg-white/5 p-2 rounded transition-colors relative border border-transparent hover:border-structure/30">
                <div className="flex justify-between items-center gap-4">
                  ...
                  <button 
                    ...
                    className={`text-[8px] font-code px-2 py-1.5 border transition-all duration-300 rounded uppercase tracking-[0.2em] select-none flex-shrink-0 ${copiedId === "rudra" ? "bg-signal text-void border-signal" : "bg-void border-structure/50 text-gray-500 hover:border-signal hover:text-signal shadow-[0_0_10px_rgba(255,255,255,0.02)]"}`}
                    style={{ minWidth: '56px' }}
                    title="Copy email"
                  >
              ...
              <div className="group/item flex flex-col gap-1 hover:bg-white/5 p-2 rounded transition-colors relative border border-transparent hover:border-structure/30">
                <div className="flex justify-between items-center gap-4">
                  ...
                  <button 
                    ...
                    className={`text-[8px] font-code px-2 py-1.5 border transition-all duration-300 rounded uppercase tracking-[0.2em] select-none flex-shrink-0 ${copiedId === "hq" ? "bg-signal text-void border-signal" : "bg-void border-structure/50 text-gray-500 hover:border-signal hover:text-signal shadow-[0_0_10px_rgba(255,255,255,0.02)]"}`}
                    style={{ minWidth: '56px' }}
                    title="Copy email"
                  >
        ...
        <div className="text-xs font-code text-gray-600 text-center md:text-right">
