...
const Systems = ({ config }) => {
  ...
  const resolvedModelPath = activeCar.modelPath
    ? (activeCar.modelPath.startsWith('/')
      ? `${import.meta.env.BASE_URL}${activeCar.modelPath.slice(1)}`
      : `${import.meta.env.BASE_URL}${activeCar.modelPath}`)
    : "";

  return (
    <section id="SYSTEMS" className="min-h-screen bg-void text-signal relative overflow-hidden flex flex-col pt-32 pb-16">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 flex flex-col items-center flex-grow">
        <div className="mb-12 flex flex-col items-center text-center">
          ...
          <h2 className="text-[11vw] sm:text-6xl md:text-[90px] lg:text-[110px] font-inter font-bold tracking-[-0.04em] text-[#555555] mb-2 leading-[0.9]">
            {data.headlinePrefix}
            <br />
        ...
        {/* TABS */}
        <div className="flex gap-4 mb-8 border-b border-structure/30 pb-2 overflow-x-auto no-scrollbar w-full justify-center">
          {cars.map((car, idx) => (
            <button
              key={idx}
        ...
        {/* CANVAS (STRICT BOUNDS) */}
        <div className="w-full max-w-4xl h-[280px] sm:h-[400px] md:h-[600px] relative rounded-xl border border-structure/30 bg-black/50 shadow-2xl overflow-hidden mb-12">
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(255,255,255,0.05)_0%,transparent_80%)] pointer-events-none z-0"></div>
          <CarCanvas modelPath={resolvedModelPath} interactive={true} />
        </div>

        {/* SPECS */}
        <div className="w-full flex-grow flex flex-col justify-end mt-auto pb-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-px bg-structure/30 border border-structure/30 backdrop-blur-md overflow-hidden">
